    ...
    class ContentContext extends LS.EventHandler {
        ...
        fromURL(url){
            this.loadPromise = new Promise(async (resolve, reject) => {
                ...
                // Extract content
                const newContent = doc.querySelector('#viewport');
                if (!newContent) {
                    kernel.errorPage(404);
                    return;
                }

                this.content = newContent;

                for (const style of newContent.getElementsByTagName('style')) {
                    this.styles.push(style);
                }

                for(const link of newContent.querySelectorAll('link[rel="stylesheet"]')) {
                    if(link.parentElement !== newContent) continue;
                    newContent.append(LS.Create("link", { href: link.href, rel: "stylesheet" }));
                    link.remove();
                }
    ...
    const kernel = new class Kernel extends LoggerContext {
        ...
        setPage(path, options = {}) {
            if(typeof path === "string") {
                ...
                // Load page dynamically if not registered
                if (!this.pageCache.has(path)) {
                    this.log("Dynamically loading page for", path);

                    const page = this.registerPage(path, {
                        url: location.origin + path
                    });

                    // TODO: This needs a lot of work before it's production ready.
                    // For now, we'll just redirect to the page instead.
                    return location.href = path;

                    // TODO: Potentially some kind of queue system for race conditions
                    // TODO: Loading indicator
                    // TODO: Error handling
                    // TODO: Resource cleanup
                    page.loadPromise?.then?.(() => {
        ...
        #setupToolbars() {
            ...
            const collapseItems = new app.utils.FrameScheduler(() => {
            ...
            const resizeMessageSwitch = new app.utils.Switch((on) => {
                if(on) {
